 ...
 * 
 * WIP:
 * - Change multiple hotkeys to use a single menu system (Keypad Enter)
 */
...
namespace HFF_SpeedTools
{
    ...
    public class SpeedTools : MonoBehaviour
    {
        ...
        private GameObject exitZoneVisual;
        private HumanAPI.LevelPassTrigger curExitZone;
        ...
        private const string modName = "Useful Speed Tools";
        ...
        {
            GUI.Label(new Rect(Screen.width - 100, 30, 100, 50), "Checkpoint: " + Game.instance.currentCheckpointNumber + " (Prev: " + this.prevCpNum + ")");
        }

        private void RenderExitZone()
        {
            if (this.renderEndZone)
            {
                /*if (FindObjectsOfType<HumanAPI.LevelPassTrigger>().Length > 1)
                {
                    DebugMessage(FindObjectsOfType<HumanAPI.LevelPassTrigger>().Length.ToString() + " loading zones on this level!");
                    DebugMessage(FindObjectsOfType<HumanAPI.LevelPassTrigger>().Length.ToString() + " loading zones on this level!", 2.0f, false);
                }*/
                if (!this.curExitZone || !this.curExitZone.gameObject.activeSelf) //if it does not exist or is not active then
                {
                    this.curExitZone = FindObjectOfType<HumanAPI.LevelPassTrigger>();
                    if (this.curExitZone && this.curExitZone.gameObject.activeSelf)
                    {
                        /*
                         * 0 = box/cube
                         * 1 = capsule
                         ...
                         */
                        int colType = -1;
                        BoxCollider boxCol = this.curExitZone.gameObject.GetComponent<BoxCollider>();
                        CapsuleCollider capsuleCol = this.curExitZone.GetComponent<CapsuleCollider>();
                        SphereCollider sphereCol = this.curExitZone.gameObject.GetComponent<SphereCollider>();
                        MeshCollider meshCol = this.curExitZone.gameObject.GetComponent<MeshCollider>();
                        ...
                        if (capsuleCol)
                            ...
                            colType = 3;
                        if (this.exitZoneVisual != this.primitives[colType])
                        {
                            //"delete" current exit zone
                            this.exitZoneVisual.transform.rotation = new Quaternion();
                            this.exitZoneVisual.transform.localScale = new Vector3(0.0f, 0.0f, 0.0f);
                            this.exitZoneVisual.transform.position = Vector3.zero;
                            //switch current exit zone visual to represent the same shape as the hitbox
                            this.exitZoneVisual = this.primitives[colType];
                        }

                        switch (colType)
                        {
                            case 0:
                                this.exitZoneVisual.transform.rotation = boxCol.transform.localRotation * this.curExitZone.gameObject.transform.rotation;
                                this.exitZoneVisual.transform.localScale = boxCol.bounds.size;
                                this.exitZoneVisual.transform.position = boxCol.bounds.center;
                                return;
                            case 1:
                                this.exitZoneVisual.transform.rotation = capsuleCol.transform.localRotation * this.curExitZone.gameObject.transform.rotation;
                                //try to figure out some way to apply capsule's "direction" to this? or it may not be necessary
                                this.exitZoneVisual.transform.localScale = new Vector3(capsuleCol.radius * 2, capsuleCol.height, capsuleCol.radius * 2);
                                //this.exitZoneVisual.transform.localScale = capsuleCol.bounds.size;
                                this.exitZoneVisual.transform.position = capsuleCol.bounds.center;
                                return;
                            case 2:
                                //this.exitZoneVisual.transform.rotation = this.curExitZone.gameObject.transform.rotation;
                                this.exitZoneVisual.transform.localScale = new Vector3(sphereCol.radius * 2, sphereCol.radius * 2, sphereCol.radius * 2);
                                //this.exitZoneVisual.transform.localScale = sphereCol.bounds.size;
                                this.exitZoneVisual.transform.position = sphereCol.bounds.center;
                                return;
                            /*case 3:
                                DebugMessage("MESH EXIT ZONE", 2.0f, false);
                                return;*/
                            default:
                                this.exitZoneVisual.transform.rotation = this.curExitZone.GetComponent<Collider>().transform.localRotation * this.curExitZone.gameObject.transform.rotation;
                                this.exitZoneVisual.transform.localScale = this.curExitZone.GetComponent<Collider>().bounds.size;
                                this.exitZoneVisual.transform.position = this.curExitZone.GetComponent<Collider>().bounds.center;
                                break;
                        }
                    }
                    else
            ...
            this.exitZoneVisual.SetActive(this.renderEndZone);
        }

        private void SetRenderLoadingZone(string txt)
        {
            if (string.IsNullOrEmpty(txt))
                this.renderEndZone = !this.renderEndZone;
            else
            {
                ...
                if (int.TryParse(words[0], out num))
                    this.renderEndZone = num != 0;
                else if (words[0].ToLower() == "true")
                    this.renderEndZone = true;
                else if (words[0].ToLower() == "false")
                    this.renderEndZone = false;
            }
            Shell.Print(string.Format("loading zone renderer {0}", this.renderEndZone ? "enabled" : "disabled"));
        }
        ...
        private void Start()
        {
            ...
            this.exitZoneVisual = this.cubePrimitive;

            foreach (GameObject theObject in this.primitives)
            {
                theObject.GetComponent<Renderer>().shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off; //so that it doesn't cast shadows on things
                StandardShaderUtils.ChangeRenderMode(theObject.GetComponent<MeshRenderer>().material, StandardShaderUtils.BlendMode.Transparent); //makes exitZoneVisual able to be transparent
                theObject.GetComponent<Renderer>().material.color = new Color(0.0f, 102.0f, 0.0f, 0.05f);
                theObject.GetComponent<Collider>().enabled = false;
                DontDestroyOnLoad(theObject);
                theObject.SetActive(this.renderEndZone);
                theObject.name = "ExitZoneVisual";
                /*this.exitZoneVisual.GetComponent<Renderer>().shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off; //so that it doesn't cast shadows on things
                StandardShaderUtils.ChangeRenderMode(this.exitZoneVisual.GetComponent<MeshRenderer>().material, StandardShaderUtils.BlendMode.Transparent); //makes exitZoneVisual able to be transparent
                this.exitZoneVisual.GetComponent<Renderer>().material.color = new Color(0.0f, 102.0f, 0.0f, 0.05f);
                this.exitZoneVisual.GetComponent<Collider>().enabled = false;
                DontDestroyOnLoad(this.exitZoneVisual);
                this.exitZoneVisual.SetActive(this.renderEndZone);
                this.exitZoneVisual.name = "ExitZoneVisual";*/
            }
            ...
            Shell.RegisterCommand("loadingzones", new System.Action<string>(this.SetRenderLoadingZone), "Shows loading zones");
            Shell.RegisterCommand("speedometer", new System.Action<string>(this.SetDisplaySpeedometer), "Enables/Disables a speedometer that tracks your speed over the past " + average_SecondsTracked + " seconds");

            DebugMessage("Press \"NumPad-Enter\" to bring up the SpeedTools menu.", 8, false);
        }
        ...
        private void Update()
        {
            ...
            if (this.renderMenu)
            {
                ...
                if (Input.GetKeyDown(KeyCode.Alpha2))
                {
                    this.renderEndZone = !this.renderEndZone;
                    RefreshMenuMessage();
                    DebugMessage(string.Format("{0} view of end zone", this.renderEndZone ? "Enabled" : "Disabled"));
                }
            ...
            bool pauseSpeedTracker = (!this.renderSpeed || !Human.instance || Game.instance.state != GameState.PlayingLevel || !(Human.instance.state == HumanState.Walk || Human.instance.state == HumanState.Jump || Human.instance.state == HumanState.Slide));
